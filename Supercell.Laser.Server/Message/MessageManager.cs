namespace Supercell.Laser.Server.Message
{
    ...
    public class MessageManager
    {
        ...
        private void TeamChangeMemberSettingsReceived(TeamChangeMemberSettingsMessage message)
        {
            ...
            {
                member.CharacterId = message.CharacterId;
                HomeMode.Home.CharacterId = message.CharacterId;
            }
            team.TeamUpdated();
        }
            ...
            if (message.Message.StartsWith("/"))
            {
                ...
                switch (cmd[0])
                {
                    case "status":
                        ...
                        response.Entry.Message = $"Server Status:\nServer Version: v{Program.SERVER_VERSION} (for 1.1714) ({Program.BUILD_TYPE})\nPlayers Online: {Sessions.Count}\n" +
        ...
        private void CancelMatchMaking(CancelMatchmakingMessage message)
        {
            ...
            MatchMakingStatusMessage status = new MatchMakingStatusMessage();
            status.Status = false;
        ...
        private void MatchmakeRequestReceived(MatchmakeRequestMessage message)
        {
            ...
            HomeMode.Home.CharacterId = message.SelectedCard;
            if (HomeMode.Home.Character.Disabled)
            {
                Connection.Send(new OutOfSyncMessage());
                return;
        ...
        private void LoginReceived(AuthenticationMessage message)
        {
            ...
            try
            {
                account.Avatar.Gold = 9999;
                account.Avatar.Diamonds = 9999;
                System.Console.WriteLine($"[DEBUG] Player Login: Gold={account.Avatar.Gold}, Gems={account.Avatar.Diamonds}, Elixir={account.Avatar.Elexir}, Chips={account.Avatar.Chips}");
                ...
                */
                ...
                MyAllianceMessage myAlliance = new MyAllianceMessage();
                myAlliance.Role = HomeMode.Avatar.AllianceRole;
                myAlliance.OnlineMembers = 0;
                myAlliance.AllianceHeader = null;
                Connection.Send(myAlliance);
