namespace Supercell.Laser.Logic.Battle.Objects
{
    ...
    public class GameObject
    {
        ...
        protected LogicVector2 Position  = new LogicVector2();
        protected int Z;
        ...
        public virtual void Encode(BitStream bitStream, bool isOwnObject, int visionTeam)
        {
            if (GameObjectManager.GameModeVariation == 6)
            {
                ...
                bitStream.WritePositiveInt(Position.X, 15);
                bitStream.WritePositiveInt(Position.Y, 16);
                bitStream.WritePositiveInt(Index, 8);
            }
            else
            {
                //Console.WriteLine("not Showdown");
                bitStream.WritePositiveInt(Position.X, 13);
                bitStream.WritePositiveInt(Position.Y, 14);
                bitStream.WritePositiveInt(Index, 7);
            }
            bitStream.WritePositiveInt(Z, 12);
